<template>
    <div>
        <div class="breadcrumb">
            <div class="container">
                <h2>
                <router-link to="/prodetail">首页</router-link>&gt;<router-link to="/prodetail">酒店预订</router-link>
                <div class="search clearfloat">
                    <button type="button"></button>
                    <input placeholder="请输入要查询的酒店" type="text"/>
                </div>
                </h2>
            </div>
        </div>
        <div class="main container">
          <ul class="product_list clearfloat" >
                ...
                <router-link router-link to="/hoteldetail">
                    <img  width="298" height="298" :src="require('../assets/images/termini/'+product.hpic)" />
                </router-link>
                <div class="pdlist_text clearfloat">
                    <h3>
                    <p>{{product.hname}}</p>
                    <span>{{product.hpubtitle}}</span>
                    </h3>
                    <!--路由传参../prodetail?pid=4-->
                    <router-link  router-link to="/hoteldetail">查看详情</router-link>
                </div>
                </li> 
          </ul>
        <!--
...
<script>
    ...
    mounted:function(){
         this.$http.get('http://localhost/mytrip/trip/data/products/hotel.php').then(
            (response)=>{
