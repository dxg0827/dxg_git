<template>
    <div>
        <div class="breadcrumb">
            <div class="container">
                <h2>
                <router-link to="/prodetail">首页</router-link>&gt;<router-link to="/prodetail">热门套餐</router-link>
                <div class="search clearfloat">
                    <button type="button"></button>
                    <input placeholder="请输入要查询的套餐" type="text"/>
                </div>
                </h2>
            </div>
        </div>
        ...
        <div class="main container">
          ...
          <ul class="product_list clearfloat" >
                ...
                <router-link router-link to="/travelmalldetail">
                    <img  width="298" height="298" :src="require('../assets/images/termini/'+product.tpic)" />
                </router-link>
                <div class="pdlist_text clearfloat">
                    <h3>
                    <p>{{product.tname}}</p>
                    <span>{{product.tpubtitle}}</span>
                    </h3>
                    <!--路由传参../prodetail?pid=4-->
                    <router-link  router-link to="/travelmalldetail">查看详情</router-link>
                </div>
                </li> 
          </ul>    
            <!--    
            <ul class="product_list clearfloat" >
                <li> 
                    ...
                    <div class="pdlist_text clearfloat"> 
                        ...
                        <router-link to="/travelmalldetail">查看详情</router-link> 
                    </div> 
                </li>
                <li> 
        ...
        <!--分页功能-->
        <div class="pages">
            <a v-show="pageNo>1" @click="--pageNo">上一页</a>
            <a @click="changePage(pno)" :class="{cur:pageNo==pno}" v-for="pno in pageList">{{pno}}</a>
            <a v-show="pageNo<pageCount" @click="++pageNo">下一页</a>
        </div>
    </div>
</template>
...
<script>
    ...
    mounted:function(){
         this.$http.get('http://localhost/mytrip/trip/data/products/travelmall.php').then(
            (response)=>{
              console.log(response.data.data);
            this.list = response.data.data;// 保存产品列表
            console.log(this.list[0].tpic);
